...
function pickRewrittenPayload(data) {
  if (data == null) return '';
  if (typeof data === 'string') return data;
  if (typeof data.text === 'string') return data.text;
  if (typeof data.rewritten === 'string') return data.rewritten;
  if (typeof data.result === 'string') return data.result;
  return '';
}

/**
 * Bottom sheet: ИИ-переписывание текста через ARIA API.
 */
  ...
  const runRewrite = useCallback(
    async (style) => {
      ...
      try {
        ...
        const res = await fetch(`${ARIA_API_URL}/rewrite`, {
          method: 'POST',
        ...
        if (!res.ok) {
          setError(typeof body?.detail === 'string' ? body.detail : 'Ошибка запроса');
          setLoading(false);
          return;
        }
        const out = pickRewrittenPayload(body);
        setRewritten(out || '');
      } catch (e) {
